# Directives & Contraintes d'Environnement — eco-antilles.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **eco-antilles.com** (eco-antilles) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `eco-antilles.com` |
| **URL Cloudflare Pages** | `https://eco-antilles.pages.dev` |
| **Dépôt GitHub** | `jc842/eco-antilles` (branche principale : `main`) |
...
| **Matomo Site ID** | `105` |
| **Matomo MTM Container** | `container_1504SCnt.js` (Container ID : `1504SCnt`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `107` |
| **Zones Publicitaires** | Header (`648`), In-Content (`649`), Mobile Sticky (`650`), Large Rectangle (`647`), Half Page (`646`), Skyscraper (`651`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_1504SCnt.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 649).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 647).
